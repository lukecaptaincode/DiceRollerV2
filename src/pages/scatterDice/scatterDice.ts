...
export class ScatterDicePage {
  ...
  deg: any;
  degDec: any;
  scatter_distance: any;

  scatter() {
    var scatterDiceContainer = document.getElementById('scatterDiceContainer');
    var scatter = document.getElementById('scatter');
    var distance_amount = document.getElementById('scatterDistance');
    ...
    if (
      this.random.nextInt32([0, 360]) % 60 == 0 ||
      this.random.nextInt32([0, 360]) % 30 == 0
    ) {
      scatter.innerHTML = 'X';
    ...
    distance_amount.innerHTML =
      'Scatter by: ' + this.random.nextInt32([2, 12]) + ' inches';
    scatterDiceContainer.style.webkitTransform =
      'rotate(' + this.deg + '.' + this.degDec + 'deg)';
    scatterDiceContainer.style.transform =
      'rotate(' + this.deg + '.' + this.degDec + 'deg)';
